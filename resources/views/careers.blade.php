        ...
        <div class="col-12 col-md-6">
            <form class="card p-5 shadow">
                ...
                <div class="mb-3">
                    ...
                    <input type="email" class="form-control" id="email" value="{{old('email') ?? Auth::user()->email}}">
                    @error('email')
                    <span clas="text-danger">{{$message}}</span>
                    @enderror
                </div>
                <div class="mb-3">
                    <label for="message" class="form-labe">Perchè vuoi candidarti per questo ruolo? Raccontacelo</label>
                    <textarea name="message" id="message" cols="30" rows="7" class="form-control">{{old('message')}}</tetarea>
                    @error('message')
                    <span class="text-danger">{{$message}}</span>
                    @enderror
                </div>
